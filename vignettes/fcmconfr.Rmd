...
This guide walks through each of these steps, from data import to visualization. It frequently references data from the `sample_fcms` object that users can access after loading `fcmconfr`.
...
Since users can make IVFN or TFN adjacency matrices by importing matrices from files, they will rarely need to create an IVFN or TFN matrix from scratch. However, the example below illustrates how `fcmconfr` creates such objects. The example we provide below is for an IVFN matrix. The approach for a TFN matrix is similar, but uses three- rather than two-element lists.
